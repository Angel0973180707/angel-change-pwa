...
// ✅ structuredClone 備援（舊 Android / WebView 會缺）
if (typeof structuredClone !== 'function') {
  window.structuredClone = (obj) => JSON.parse(JSON.stringify(obj));
}

// ✅ replaceAll 備援（舊 Android 會缺，缺了會整支掛）
...
function bindTimer() {
  ...
  if (s1_seconds) s1_seconds.addEventListener('input', () => {
    if (!timer.running) {
      const lab = $('#s1_secondsLabel');
      if (lab) lab.textContent = s1_seconds.value;
  ...
  if (btnTimerReset) btnTimerReset.addEventListener('click', () => {
    ...
    const lab = $('#s1_secondsLabel');
    if (lab) lab.textContent = String(state.s1.seconds || 45);
    toast('已重設');
  });
}
...
function bindQuotes() {
  ...
  if (quotesEnabled) quotesEnabled.addEventListener('change', () => {
    ...
    saveQuotes();
    const block = $('#quotesBlock');
    if (block) block.style.display = quotes.enabled ? '' : 'none';
